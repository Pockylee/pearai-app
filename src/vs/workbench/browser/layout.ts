	...
	implements IWorkbenchLayoutService
{
	...
	protected restoreParts(): void {
		...
		// modified restore Auxliary Bar
		// Restore Auxiliary Bar
		const logPrefix = "%c[Custom Log]";
		const logStyle = "color: orange; font-weight: bold;";
		...
		layoutReadyPromises.push(
			(async () => {
				...
				await restoreDefaultViewsPromise;
				if (!this.state.initialization.views.containerToRestore.auxiliaryBar) {
					// Set PearAI as default if no auxiliary bar view container is set to restore
					const pearAiViewContainerId = "PearAI";

					// Attempt to restore the view container from storage
					const viewContainerToRestore = this.storageService.get(
						AuxiliaryBarPart.activePanelSettingsKey,
						StorageScope.WORKSPACE,
						pearAiViewContainerId,
					);

					// Log the view container being restored
					console.log(
						logPrefix,
						logStyle,
						"View container to restore:",
						viewContainerToRestore,
					);

					// If a view container is found in storage, use it; otherwise, use PearAI
					if (viewContainerToRestore) {
						this.state.initialization.views.containerToRestore.auxiliaryBar =
							viewContainerToRestore;
					} else {
					...
					// Ensure the auxiliary bar is visible
					this.stateModel.setRuntimeValue(
						LayoutStateKeys.AUXILIARYBAR_HIDDEN,
						false,
					);
				...
				mark("code/didRestoreAuxiliaryBar");
				console.log(logPrefix, logStyle, "Auxiliary Bar restoration complete.");
			})(),
		);
		...
		console.log(
			logPrefix,
			logStyle,
			"Defaulting to PearAI view container:",
			"PearAI",
		);
